...
do
  dir=/home2/danr/scripts/phylo_spatial/multi_runs/
  filename_base=callExtractBranchRanges_
  startnum=$[num*5+1]
  endnum=$[num*5+5]
  suffix=$startnum"-"$endnum".r"

  filename=$dir$filename_base$suffix

echo $filename

echo "#! /usr/local/lib64/R/bin/Rscript --vanilla
 ...
 #phylogeny_filename     <- "FritzTree.rs200k.100trees_1st_tree.tre"
  ...
  output_type             <- "Marxan"
  land_cap                <- TRUE
  first_tree		     <-" $startnum >> $filename
echo "  number_of_trees         <- 5
