...
arcpy.env.grid_resolution = 0.01
# Load the edit rule feauture class
print "\nLoading the edit rules\n"
rules_fc_path = rules_gdb + "\\" + rules_fc
rule_cursor   = arcpy.da.SearchCursor(rules_fc_path, ["OBJECTID","SHAPE@","Action","Species","Distance"])
for row in rule_cursor:
        print(row)  # for debugging - remove once working
        rule_poly = row[1]
        rule_action = row[2]
        rule_taxon= row[3]
        rule_distance=row[4]
        ...
        if rule_action == "fade" and rule_distance > 0:
            ...
            print "\nAbout to do " + rule_taxon + ". Include areas within polygon\n"
